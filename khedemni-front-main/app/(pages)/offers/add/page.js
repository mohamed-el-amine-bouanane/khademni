...
const AddOffer = () => {
  ...
  const handleSubmit = async(e) => {
    ...
    else if(images.length ==0)
    {
      ...
      try{
        ...
        const result = await api.post('/api/tasks/', form, 
        {headers: {
        ...
        {
          router.push('/users/'+authContext.authState.user.id); // Replace with your actual success page path
        }
      }
      catch (error) {
  ...
  <div>
   {(!isLoading && !Err) ? <div className="px-20 flex justify-center items-center flex-col">
      <h3 className="text-[#27419E] text-[40px] font-bold mt-6">Add New Offer</h3>
